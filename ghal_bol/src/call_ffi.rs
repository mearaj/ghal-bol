//! FFI: per-call media keys for Flutter WebRTC FrameCryptor (identity secp256k1).
...
use crate::call_media_key::derive_call_media_keys_from_identity;
...
/// JSON in: `{ "call_id": "…", "peer_public_key_hex": "66-hex…" }`
/// JSON out: `{ "ok": true, "key_hex", "ratchet_salt_hex", "local_public_key_hex" }`
///
...
pub unsafe extern "C" fn ghal_bol_ffi_call_media_key_hex(config_json_utf8: *const c_char) -> *mut c_char {
    let run = || -> *mut c_char {
        ...
        let keys = match derive_call_media_keys_from_identity(&ident, peer, call_id) {
            Ok(k) => k,
            Err(e) => return json_err(e),
        };
        json_ok(serde_json::json!({
            ...
            "local_public_key_hex": local_hex,
        }))
    };
    run()
}
