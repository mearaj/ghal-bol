...
fn spawn_leave_read_ack_drain(session: Arc<SessionState>, writers: StreamWriters, left: PeerId) {
...
use libp2p::swarm::{ConnectionId, DialError, NetworkBehaviour, Swarm, SwarmEvent};
use libp2p::tcp;
use libp2p::yamux;
...
const LAN_DIAL_THROTTLE_URGENT_MS: i64 = 8_000;
...
struct SessionState {
    ...
    /// Live relay-circuit `ConnectionId`s per DM peer — closed when direct LAN is up.
    ...
    coord_lookup_last_category:
        RwLock<HashMap<String, super::connectivity_diag::CoordLookupCategory>>,
...
impl SessionState {
    ...
    /// Defer coord relay while a direct LAN TCP dial is in flight, direct link is up, or
    /// handover is waiting for first mDNS after candidate purge.
    /// Do not defer merely because stale addrs remain in the candidate set (TRANSPORT.md).
    fn should_defer_coord_relay_for_lan(
        ...
        // No mDNS candidate yet while link is down — give LAN discovery a chance before relay.
        self.is_dm_contact(peer)
            && !self.lan_candidates_exhausted(peer)
            && self.peer_mdns_lan_addr(peer).is_none()
            && (!swarm.is_connected(&peer) || !self.dm_peer_stream_up(peer))
    }
    ...
    fn peer_coord_absent_never_connected(&self, pk_hex: &str, peer: PeerId) -> bool {
        self.coord_lookup_category_for_pk(pk_hex)
            == Some(super::connectivity_diag::CoordLookupCategory::PeerNotOnCoord)
    ...
    fn peer_mdns_lan_addr(&self, peer: PeerId) -> Option<Multiaddr> {
        ...
        v.iter()
            ...
            .find(|ma| super::network_transport::is_publishable_listen_addr(ma))
    ...
    fn defer_coord_for_lan_rediscovery(&self, peer: PeerId, now_ms: i64) -> bool {
        ...
        self.peer_mdns_lan_addr(peer).is_none() && wifi_lan_handover_active(self)
    }
    ...
    fn drain_dm_relay_connection_ids(&self, peer: PeerId) -> Vec<ConnectionId> {
        let Ok(mut m) = self.dm_relay_conn_ids.write() else {
            return Vec::new();
        };
        m.remove(&peer)
    ...
    fn mark_dm_reconnect_urgent_unless_live_direct_stream(&self) {
        for peer in self.dm_peer_ids() {
            if self.dm_peer_stream_up(peer) && self.peer_has_direct_connection(peer) {
                continue;
            }
            if let Some(pk) = self
    ...
    /// Throttle coord HTTP for peers absent from coord (404). Urgent reconnect and presence-wake
    /// bypass; otherwise retry on backoff (not permanent silence — remote may register anytime).
    ...
    fn clear_peer_coord_absent_state(&self, pk_hex: &str) {
        ...
        self.clear_coord_lookup_backoff(pk);
        if let Ok(mut m) = self.coord_lookup_last_category.write() {
            if m.get(pk) == Some(&super::connectivity_diag::CoordLookupCategory::PeerNotOnCoord) {
                m.remove(pk);
            }
        }
    }
        ...
        .filter(|p| {
            session.is_bootstrap_peer(**p)
                ...
                && !swarm.listeners().any(|l| {
                    super::network_transport::is_coord_ipv4_relay_listen(l)
...
fn sync_published_listen_from_swarm(session: &SessionState, swarm: &Swarm<ChatBehaviour>) -> bool {
    ...
    v.retain(|ma| {
        if super::network_transport::is_relay_circuit_multiaddr(ma) {
            return live.contains(ma) && super::network_transport::is_coord_ipv4_relay_listen(ma);
        }
        true
    });
    for ma in live {
...
async fn maybe_refresh_ghalbol_relay(
    ...
    let all_relays =
        tokio::task::spawn_blocking(move || crate::coord_runtime::fetch_all_ghalbol_relays(cache))
            .await
            .ok()
            .unwrap_or_default();
...
fn network_profile_for_swarm(
    ...
    super::network_transport::effective_network_profile(detected, swarm_has_lan_dm_listen(swarm))
}
...
/// On LAN: full handover when link down + no mDNS candidate; stream reopen only when candidate exists.
fn lan_handover_upkeep_if_needed(swarm: &mut Swarm<ChatBehaviour>, session: &SessionState) {
    ...
    if needs_mdns_nudge {
        // Full handover (fresh listen + force mDNS), not soft restart — matches cold start.
        kick_lan_dm_rediscovery_after_handover(
            swarm,
            session,
            "link down, no mDNS candidate yet",
            false,
        );
        return;
    }
    let in_grace = session.within_lan_handover_grace(now_ms);
    let stream_down = session
        .dm_peer_ids()
        .iter()
        .any(|p| session.should_dial_libp2p_peer(*p) && !session.dm_peer_stream_up(*p));
    if in_grace && stream_down && session.should_run_lan_recovery(now_ms) {
...
async fn process_outbound_cmd(
    ...
    let (peer, frame, done) = match cmd {
        ...
        OutboundCmd::SendText {
            ...
            if !swarm.is_connected(&peer) {
                if pk.len() == 66 && !session.peer_coord_absent_never_connected(pk, peer) {
                    session.mark_dm_reconnect_urgent(pk);
                }
                notify_coord_lookup();
            }
...
/// True when libp2p still has an outbound dial pending for this peer (probe only — no dial issued).
fn libp2p_peer_dial_pending(swarm: &mut Swarm<ChatBehaviour>, peer: PeerId) -> bool {
    if swarm.is_connected(&peer) {
        return false;
    }
    matches!(
...
fn lan_dial_expired_coord_fallback(
    ...
    if session.defer_coord_for_lan_rediscovery(peer, now_ms) {
        native_log::info(
            "mdns",
            format!("LAN dial window expired for {peer} — waiting for fresh mDNS (coord deferred)"),
        );
...
fn close_dm_relay_connections_for_peer(
    swarm: &mut Swarm<ChatBehaviour>,
    session: &SessionState,
    peer: PeerId,
) {
    for conn_id in session.drain_dm_relay_connection_ids(peer) {
        if swarm.close_connection(conn_id) {
            native_log::info(
                "swarm",
                format!("closed relay circuit to {peer} (direct LAN active)"),
            );
        }
    }
}

/// Relay is redundant only on active Wi‑Fi LAN with a live direct link to the peer.
/// After `left LAN`, stale `peers_direct_conns` counts must not tear down WAN relay circuits.
fn prefer_direct_dm_path_over_relay(session: &SessionState, peer: PeerId) -> bool {
    session.network_profile_snapshot().has_active_lan()
        && session.peer_on_local_lan(peer)
        && session.peer_has_direct_connection(peer)
}

fn dm_connection_is_relay(endpoint: &ConnectedPoint) -> bool {
...
fn dial_dm_peer_addr(
    ...
    if is_relay && session.should_defer_coord_relay_for_lan(swarm, peer, now) {
        return;
    }
    if !is_relay {
    ...
    if crate::coord_runtime::coord_is_configured() && session.prefers_mobile_coord_strategy() {
        let on_lan = session.peer_on_local_lan(peer)
            || (session.network_profile_snapshot().has_active_lan()
                && super::network_transport::ipv4_from_ma_str(&addr.to_string()).is_some_and(
                    |ip| ip.is_private() && !super::network_transport::is_cgnat_ipv4(ip),
                ));
        if !is_relay && !on_lan {
            let now = chrono_now_ms();
            if session.should_log_dial_skip(peer, now, 8_000) {
...
fn dial_mdns_lan_addr(
    ...
    if session.circuit_dial_in_flight_blocks(peer, now) {
        ...
        if wifi_lan_handover_active(session) || session.network_profile_snapshot().has_active_lan()
        {
...
fn dial_additive_dm_addr(
    ...
    if is_relay {
        let pk_hex = session
            .dm_peer_for_libp2p(peer)
            .and_then(|d| d.public_key_hex.clone());
        if dm_peer_chat_link_stable(swarm, session, peer, pk_hex.as_deref(), chrono_now_ms()) {
            return;
        }
    }
    if is_relay && prefer_direct_dm_path_over_relay(session, peer) {
        return;
    }
    if !is_relay && !is_tcp_multiaddr(&addr) {
        return;
    ...
    match event {
        ...
        SwarmEvent::ConnectionClosed {
            ...
            if session.is_bootstrap_peer(peer_id) {
                ...
                let is_relay = dm_connection_is_relay(&endpoint);
                if is_relay {
        ...
        SwarmEvent::ConnectionEstablished {
            ...
            if session.is_dm_contact(peer_id) {
                ...
                let is_relay = dm_connection_is_relay(&endpoint);
                if is_relay {
                    if prefer_direct_dm_path_over_relay(session, peer_id) {
                        native_log::info(
                            "swarm",
                            format!("drop redundant relay link to {peer_id} (direct LAN up)"),
                        );
                    ...
                    close_dm_relay_connections_for_peer(swarm, session, peer_id);
        ...
        SwarmEvent::Behaviour(ChatBehaviourEvent::Relay(
            ...
            // Relay circuit live — publish WAN presence and wake peer rediscovery.
            notify_dm_presence_wake();
            finish_wan_recovery_if_ready(session, swarm);
        }
...
/// Coalesce reservation-accepted + coord-register + ui-visible into one wake notify.
const PRESENCE_WAKE_NOTIFY_DEBOUNCE_MS: i64 = 3_000;
/// Minimum gap between full `wake_all_dm_peers_rediscovery` runs.
const PRESENCE_WAKE_RUN_DEBOUNCE_MS: i64 = 4_000;
/// Repeat `peer_not_on_coord` lookup lines at INFO at most once per interval.
const COORD_PEER_NOT_ON_COORD_LOG_MIN_MS: i64 = 60_000;
...
async fn coord_lookup_dm_peer(
    ...
    // Protonet: open stream → no coord lookup, no disconnect, no additive dials.
    if dm_peer_chat_link_stable(swarm, session, target, Some(pk), now_ms) {
        return;
    }
    let absent_never = session.peer_coord_absent_never_connected(pk, target);
    let urgent = session.is_pk_reconnect_urgent(pk, now_ms) && !absent_never;
    ...
    let mut skip_coord_http = if absent_never {
        session.should_skip_coord_lookup_pk(pk, now_ms)
    } else if !urgent {
        session.should_skip_coord_lookup_pk(pk, now_ms)
    } else {
        false
            ...
            _ = dm_upkeep_tick.tick() => {
                ...
                if crate::coord_runtime::coord_is_configured() && !handover_grace {
                    ...
                    for pk in session.dm_public_keys() {
                        ...
                        let outbox_waiting = session.has_pending_outbox_for_pk(&pk);
                        if coord_lookup_wake
                            || outbox_waiting
                            || session.should_coord_lookup_pk(
                                &pk,
                                now_ms,
            ...
            ev = swarm.select_next_some() => {
                if let SwarmEvent::ConnectionEstablished { peer_id, connection_id, endpoint, .. } = &ev {
                    ...
                    if session.is_bootstrap_peer(*peer_id) {
                        ...
                        let pid = *peer_id;
                        let is_relay = dm_connection_is_relay(endpoint);
                        if is_relay && prefer_direct_dm_path_over_relay(session.as_ref(), pid) {
                            native_log::info(
                                "swarm",
                                format!("ignore relay link to {pid} — direct LAN already up"),
                            );
                            let _ = swarm.close_connection(*connection_id);
                        } else {
                        if !is_relay {
                            session.note_connection_path(pid, false);
                            close_dm_relay_connections_for_peer(
                                &mut swarm,
                                session.as_ref(),
                                pid,
                            );
...
mod mdns_connect_tests {
    ...
    fn test_session() -> SessionState {
        let (_ks, identity) =
            crate::keystore_v1::create_keystore_v1("test-mdns-port", None).unwrap();
        SessionState::new(
            identity,
            &[],
            HashSet::new(),
            None,
            None,
            network_transport::detect_local_network_profile(),
            None,
            None,
        )
        .unwrap()
    ...
    #[test]
    fn merge_keeps_all_lan_tcp_until_expired_or_dial_fail() {
        let session = test_session();
        let peer = PeerId::random();
        let first: Multiaddr =
            "/ip4/192.168.1.38/tcp/43377/p2p/16Uiu2HAm5zdGNzac9hYfCNQZTnANbxWytcMty9twy7u942fT7MCk"
                .parse()
                .unwrap();
        let second: Multiaddr =
            "/ip4/192.168.1.38/tcp/33527/p2p/16Uiu2HAm5zdGNzac9hYfCNQZTnANbxWytcMty9twy7u942fT7MCk"
                .parse()
                .unwrap();
        assert!(session.merge_mdns_lan_candidate(peer, &first));
        assert!(session.merge_mdns_lan_candidate(peer, &second));
        let addrs = session
            .peer_mdns_lan_candidate_addrs
            .read()
            .unwrap()
        ...
        assert_eq!(session.peer_mdns_lan_addr(peer), Some(first.clone()));
        session.remove_mdns_lan_candidate(peer, Some(&first));
        assert_eq!(session.peer_mdns_lan_addr(peer), Some(second));
    }

    #[test]
    fn peer_mdns_lan_addr_failover_uses_discovery_order_not_port() {
        let session = test_session();
        let peer = PeerId::random();
        let first: Multiaddr =
            "/ip4/192.168.1.38/tcp/45735/p2p/16Uiu2HAm5zdGNzac9hYfCNQZTnANbxWytcMty9twy7u942fT7MCk"
                ...
                .unwrap();
        let second: Multiaddr =
            "/ip4/192.168.1.37/tcp/36131/p2p/16Uiu2HAm5zdGNzac9hYfCNQZTnANbxWytcMty9twy7u942fT7MCk"
                .parse()
                .unwrap();
        assert!(session.merge_mdns_lan_candidate(peer, &first));
        assert!(session.merge_mdns_lan_candidate(peer, &second));
        assert_eq!(session.peer_mdns_lan_addr(peer), Some(first));
    }

    #[test]
    fn merge_skips_non_lan_tcp_addrs() {
        let session = test_session();
        let peer = PeerId::random();
        let relay: Multiaddr =
            "/dns4/coord.ghalbol.com/tcp/4002/p2p/12D3KooWKUiRKKzspUQShSLWVwxgp1HnSAs3EgDLCQbXn5iGHGhF/p2p-circuit/p2p/16Uiu2HAm5zdGNzac9hYfCNQZTnANbxWytcMty9twy7u942fT7MCk"
                .parse()
                .unwrap();
        assert!(!session.merge_mdns_lan_candidate(peer, &relay));
    }
