...
fn spawn_unlock_reminder() {
    #[cfg(target_os = "linux")]
    {
        use ghal_bol::detect_keystore_app_namespace;
        use ghal_bol::daemon::{ui_presence_active, ui_session_active};
        use ghal_bol::wake_for_unlock;
        use ghal_bol::session_unlocked;
        ...
        std::thread::spawn(move || {
            ...
            eprintln!(
                "ghal_bol_daemon: unlock wake — opening UI for password (app_id={app_id}, grace_secs={grace_secs})"
            );
...
fn post_unlock_notification(app_id: &str) {
    ...
    handle.wait_for_action(move |action| {
        ...
        if ghal_bol::session_unlocked() {
            return;
        }
        ghal_bol::wake_for_unlock(&launch_id);
    });
}
