//! ML-DSA-65 (FIPS 204) identity helpers — **signing / auth only** (not transport ciphertext).
...
mod tests {
    ...
    fn sign_verify_roundtrip() {
        let seed = generate_secret_seed();
        let sk = signing_key_from_seed_bytes(&seed).unwrap();
        let pk = public_key_bytes_from_seed(&seed).unwrap();
        let msg = b"ghal_bol ml-dsa-65 auth";
