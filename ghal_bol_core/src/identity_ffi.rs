//! FFI: multi-algorithm identity parse / compare (`ghal_bol_core_ffi_identity_*`).
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghal_bol_core_ffi_identity_supported_algorithms() -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghal_bol_core_ffi_identity_validate_import_secret(
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghal_bol_core_ffi_identity_parse(wire_utf8: *const c_char) -> *mut c_char {
    let run = || -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghal_bol_core_ffi_identity_normalize(
...
mod identity_ffi_tests {
    ...
    fn supported_algorithms_are_secp_ed25519_ecdsa_p256() {
        let raw = unsafe {
            let ptr = ghal_bol_core_ffi_identity_supported_algorithms();
    ...
    fn validate_import_secret_rejects_ml_dsa65() {
        ...
        let raw = unsafe {
            let ptr = ghal_bol_core_ffi_identity_validate_import_secret(cfg_s.as_ptr());
            let s = std::ffi::CStr::from_ptr(ptr).to_string_lossy().into_owned();
            let _ = CString::from_raw(ptr);
            s
        };
