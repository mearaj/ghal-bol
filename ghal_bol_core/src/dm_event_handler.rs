...
use crate::dm_transcript_store::{
    StoredChatLine, append_if_new, load_merged, patch_outgoing_delivery, patch_outgoing_received_at,
};
...
fn outbound_delivery_for_ack(msg_kind: &str) -> &'static str {
    match msg_kind.trim() {
        "ack_read" => "read",
        "attachment_complete" => "downloaded",
        _ => "delivered",
    }
}
...
/// Persist inbound voice as soon as the DM stream/delivery worker decrypts it.
pub fn persist_inbound_voice_on_wire(
    from_peer_id: &str,
    message_id: &str,
    duration_ms: u32,
    audio_path: &str,
...
/// Persist an inbound attachment as soon as the sealed DM is accepted.
/// When `local_path` is set (mailbox delivery / LAN inline), the file is already on disk.
pub fn persist_inbound_attachment_offer_on_wire(
    from_peer_id: &str,
    message_id: &str,
    file_name: &str,
    mime_type: &str,
    ...
    let preview = crate::attach_v1::attachment_preview(file_name);
    ...
    if let Some(path) = local_path.map(str::trim).filter(|s| !s.is_empty()) {
        if let Some(obj) = ev.as_object_mut() {
            obj.insert("local_path".to_string(), serde_json::json!(path));
        }
    }
    apply_p2p_event_json(&ev)
}
...
pub fn apply_p2p_event_json(ev: &Value) -> bool {
    ...
    match kind {
        ...
        "dm_message" => {
            ...
            if msg_kind == "voice" {
                return apply_inbound_voice(&ns, ev);
            }
            if msg_kind == "attachment_offer" {
                return apply_inbound_attachment_offer(&ns, ev);
            }
            if msg_kind == "availability_status" {
                return apply_inbound_availability_status(&ns, ev);
            }
            if msg_kind == "ack_received" || msg_kind == "ack_read" {
                return apply_inbound_ack(&ns, ev, msg_kind);
            }
            if msg_kind == "attachment_complete" {
                return apply_inbound_ack(&ns, ev, msg_kind);
            }
            flow_log::warn(
                "DM/store",
...
fn apply_inbound_availability_status(ns: &str, ev: &Value) -> bool {
    ...
    match set_contact_availability_status(ns, &sender_pk, status.as_deref()) {
        Ok(_) => true,
        Err(e) => {
            flow_log::warn(
                "DM/store",
                format!(
                    "availability_status skipped pk={}: {e}",
                    short_hex(&sender_pk)
                ),
            );
            false
...
fn apply_inbound_voice(ns: &str, ev: &Value) -> bool {
    ...
    let audio_path = ev
        ...
        .map(str::trim)
        .unwrap_or("");
    if !msg_id.is_empty() && audio_path.is_empty() {
        // Poll replay after wire persistence carries metadata only; transcript already has the path.
        return false;
    }
    if msg_id.is_empty() || from_key.is_empty() {
        flow_log::warn(
            "DM/store",
            format!("inbound voice dropped: missing id/from/audio_path (id={msg_id})"),
        );
        return false;
    }
    if is_valid_public_key_hex(&my_pk) && sender_pk == my_pk {
        flow_log::info(
            "DM/store",
            format!("inbound voice ignored: own message id={msg_id}"),
        );
        return false;
    }
    if contact_is_blocked(ns, &sender_pk, wire_peer_id) {
        flow_log::info(
            "DM/store",
            format!(
                "inbound voice ignored: blocked sender pk={}",
                short_hex(&sender_pk)
            ),
        );
        return false;
    ...
    if poll_replay {
        ...
        flow_log::info(
            "DM/store",
            format!("inbound voice replay id={msg_id} conv={conv_key} — skip unread/append"),
        );
    }
    ...
    let preview = ev
        ...
        .map(str::to_string)
        .unwrap_or_else(|| crate::voice_msg_v1::voice_preview(duration_ms.unwrap_or(1)));
    ...
    if is_valid_public_key_hex(&sender_pk) {
        if find_by_public_key(ns, &sender_pk).ok().flatten().is_none() {
            flow_log::info(
                "DM/store",
                format!(
                    "auto-create contact from inbound voice pk={}",
                    short_hex(&sender_pk)
                ),
            );
            let _ = upsert_contact(
        ...
        if !poll_replay {
            let bump_unread = !skip_unread;
            let _ = record_inbound_preview(ns, &sender_pk, &preview, bump_unread, Some(at));
            changed = true;
        }
    }
    ...
    let line = StoredChatLine {
        ...
        size_bytes: None,
        local_path: Some(audio_path.to_string()),
    };
    if poll_replay {
        return changed;
    }
    match append_if_new(ns, &conv_key, line) {
        Ok(()) => {
            flow_log::info(
                "DM/store",
                format!(
                    "transcript append inbound voice id={msg_id} conv={conv_key} from={line_from}"
                ),
            );
            changed = true;
        }
        Err(e) => {
            flow_log::warn(
                "DM/store",
                format!("transcript append voice failed id={msg_id} conv={conv_key}: {e}"),
            );
        }
    }
    changed
...
fn apply_inbound_attachment_offer(ns: &str, ev: &Value) -> bool {
    ...
    let size_bytes = ev.get("size_bytes").and_then(|v| v.as_u64());
    if msg_id.is_empty() || from_key.is_empty() {
        flow_log::warn(
            "DM/store",
            format!("inbound attachment dropped: missing id/from (id={msg_id})"),
        );
        return false;
    }
    if is_valid_public_key_hex(&my_pk) && sender_pk == my_pk {
    ...
    if contact_is_blocked(ns, &sender_pk, wire_peer_id) {
        flow_log::info(
            "DM/store",
            format!(
                "inbound attachment ignored: blocked sender pk={}",
                short_hex(&sender_pk)
            ),
        );
        return false;
    ...
    let preview = ev
        ...
        .filter(|s| !s.trim().is_empty())
        .map(str::to_string)
        .unwrap_or_else(|| crate::attach_v1::attachment_preview(file_name));
    let mut changed = false;
    if is_valid_public_key_hex(&sender_pk) {
        if find_by_public_key(ns, &sender_pk).ok().flatten().is_none() {
            let _ = upsert_contact(
        ...
        let at = ev
            ...
            .unwrap_or_else(now_ms);
        if !poll_replay {
            let _ = record_inbound_preview(ns, &sender_pk, &preview, !skip_unread, Some(at));
            changed = true;
        }
    }
    let created_at_ms = ev
    ...
    let line = StoredChatLine {
        local_id: format!("bg-{created_at_ms}-{}", from_key.len()),
        text: preview,
        outgoing: false,
        from: Some(line_from.clone()),
        message_id: Some(msg_id.to_string()),
    ...
    match append_if_new(ns, &conv_key, line) {
        Ok(()) => {
            flow_log::info(
                "DM/store",
                format!("transcript append inbound attachment id={msg_id} conv={conv_key}"),
            );
            changed = true;
        }
        Err(e) => {
            flow_log::warn(
                "DM/store",
                format!("transcript append attachment failed id={msg_id} conv={conv_key}: {e}"),
            );
        }
    }
    changed
...
fn apply_inbound_ack(ns: &str, ev: &Value, msg_kind: &str) -> bool {
    ...
    if (msg_kind == "ack_read" || msg_kind == "attachment_complete") && has_outgoing {
