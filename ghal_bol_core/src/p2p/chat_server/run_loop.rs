...
pub async fn run_gossip_chat_node_with_std_io(
    ...
    let mut ghal_bol_relay_initial: Option<(PeerId, Vec<String>)> = None;
    ...
    if coord_only {
        ...
        let all_relays = tokio::task::spawn_blocking(|| {
            crate::coord_runtime::fetch_all_ghal_bol_relays(false)
        })
        .await
        .ok()
        .unwrap_or_default();
        for (relay_peer, relay_addrs) in &all_relays {
            ...
            if let Ok(relay_pid) = relay_peer.parse::<PeerId>() {
                if ghal_bol_relay_initial.is_none() {
                    ghal_bol_relay_initial = Some((relay_pid, relay_addrs.clone()));
                }
            }
        }
        native_log::info(
    ...
    if let Some((relay_pid, _)) = &ghal_bol_relay_initial {
    ...
    if crate::text_transport::p2p_wan_text_enabled()
        && let (Some(path), Some(ns)) = (&config.transcript_path, &config.app_namespace)
    {
    ...
    loop {
        ...
        select! {
            ...
            _ = stream_upkeep_tick.tick() => {
                ...
                if crate::text_transport::p2p_wan_text_enabled()
                    && session.pending_read_ack_len() > 0
                {
                ...
                } else if crate::text_transport::wan_text_via_delivery_server()
                    && crate::text_transport::p2p_lan_text_enabled()
                    && session.pending_read_ack_len() > 0
                {
                    let connected_lan: Vec<PeerId> = session
                        .connected_peers()
                        .into_iter()
                        .filter(|p| {
                            session.is_dm_contact(*p)
                                && swarm.is_connected(p)
                                && session.peer_on_local_lan(*p)
                        })
                        .collect();
                    if !connected_lan.is_empty() {
                        let session3 = Arc::clone(&session);
                        let writers3 = Arc::clone(&writers);
                        tokio::spawn(async move {
                            run_ack_upkeep(session3, writers3, &connected_lan).await;
                        });
                    }
                }
            }
            _ = dm_upkeep_tick.tick() => {
                let p2p_wan_text = crate::text_transport::p2p_wan_text_enabled();
                if p2p_wan_text || crate::text_transport::p2p_lan_text_enabled() {
                ...
                if p2p_wan_text {
                    ...
                    reopen_all_zombie_dm_mux(session.as_ref(), &writers);
                    apply_pending_dm_link_resets(&mut swarm, session.as_ref(), &writers);
                }
                let coord_lookup_wake = take_coord_lookup_notify();
                if p2p_wan_text
                    && take_dm_presence_wake_notify()
                    && session.should_run_presence_wake(now_ms)
                {
                    native_log::info(
                        "net",
                ...
                // Coord register + lookup: WAN voice/video + LAN paths (not WAN text outbox).
                ...
                if p2p_wan_text {
                    ...
                    .await;
                } else if crate::text_transport::p2p_lan_text_enabled() {
                    let connected_lan: Vec<PeerId> = connected_now
                        .iter()
                        .copied()
                        .filter(|p| session.peer_on_local_lan(*p))
                        .collect();
                    if !connected_lan.is_empty() {
                        if let (Some(path), Some(ns)) = (
                            &session.transcript_path,
