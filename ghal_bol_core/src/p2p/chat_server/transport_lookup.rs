...
/// Whether this contact has an active LAN libp2p path suitable for P2P text (mDNS / direct TCP).
pub fn contact_has_lan_p2p_text_path(identity_wire: &str) -> bool {
    if !crate::text_transport::p2p_lan_text_enabled() {
        return false;
    }
    let wire = match crate::public_key_util::normalize_contact_identity_wire(identity_wire) {
        Ok(w) => w,
        Err(_) => return false,
    };
    let session = match active_mx().read().ok().and_then(|g| g.upgrade()) {
        Some(s) => s,
        None => return false,
    };
    let peer = match session.resolve_send_peer(&wire) {
        Some(p) => p,
        None => return false,
    };
