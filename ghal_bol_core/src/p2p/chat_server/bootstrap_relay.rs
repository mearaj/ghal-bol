...
fn redial_ghal_bol_bootstrap_from_cache(
    swarm: &mut Swarm<ChatBehaviour>,
    session: &SessionState,
    log_label: &str,
) -> bool {
    let Some((relay_peer, addrs)) = session
        .ghal_bol_relay_state
        .read()
        .ok()
        .and_then(|g| g.clone())
    else {
...
fn ghal_bol_relay_peer(session: &SessionState) -> Option<PeerId> {
    session
        .ghal_bol_relay_state
        .read()
        .ok()
...
fn ensure_wan_relay_circuit(
    ...
    if !bootstrap_up {
        ...
        let profile = session.network_profile_snapshot();
        if profile.on_mobile_data_path() || profile.needs_relay_for_wan() {
            if try_ghal_bol_probe_style_circuit_listen(swarm, session, force) {
                native_log::info(
                    "relay",
...
fn relays_to_try_for_reservation(
    ...
    let Some(ghalbol) = ghal_bol_relay_peer(session) else {
        return eligible;
    };
...
fn try_ghal_bol_probe_style_circuit_listen(
    swarm: &mut Swarm<ChatBehaviour>,
    session: &SessionState,
    _force: bool,
) -> bool {
...
fn try_relay_reservations(
    ...
    let now_ms = chrono_now_ms();
    if !force && any_relay_reserve_in_flight(session, now_ms) {
        if session.should_log_dial_skip(ghal_bol_relay_peer(session).unwrap_or(PeerId::random()), now_ms, 8_000) {
...
fn handle_lan_path_restored(
    ...
    refresh_coord_presence_soft(swarm, session, coord_relays);
    let need_bootstrap = ghal_bol_relay_peer(session)
...
fn interface_drift_should_defer_full_kick(
    ...
    let bootstrap_tracked = ghal_bol_relay_peer(session)
        .map(|relay| has_tracked_bootstrap_tcp(session, relay))
        .unwrap_or(false);
...
fn kick_relay_ipv4_circuit_recovery(
    ...
    if let Some(ghalbol) = ghal_bol_relay_peer(session) {
        clear_relay_reserve_in_flight(session, ghalbol);
        if let Ok(mut g) = session.relay_reserve_requested.write() {
            g.remove(&ghalbol);
        }
...
const GHAL_BOL_RELAY_REFETCH_MS: i64 = 30_000;
/// Aggressive refetch while no relay dial addr is known (coord may have just enabled relay).
const GHAL_BOL_RELAY_REFETCH_EMPTY_MS: i64 = 5_000;
...
fn replace_ghal_bol_relay_nodes_in_coord_relays(
    coord_relays: &mut Vec<(PeerId, Multiaddr)>,
    relay_peer: PeerId,
    nodes: &[(PeerId, Multiaddr)],
) {
...
async fn maybe_refresh_ghal_bol_relay(
    swarm: &mut Swarm<ChatBehaviour>,
    session: &SessionState,
    coord_relays: &mut Vec<(PeerId, Multiaddr)>,
    force: bool,
    ...
    if !force {
        ...
        let min_gap = if need_relay {
            GHAL_BOL_RELAY_REFETCH_EMPTY_MS
        } else {
            GHAL_BOL_RELAY_REFETCH_MS
        };
    ...
    let all_relays =
        tokio::task::spawn_blocking(move || crate::coord_runtime::fetch_all_ghal_bol_relays(force))
            .await
            .ok()
            .unwrap_or_default();
    if let Ok(mut g) = session.ghal_bol_relay_last_fetch_ms.write() {
        *g = now;
    }
    ...
    for (peer_str, addrs) in &all_relays {
        ...
        if force {
            replace_ghal_bol_relay_nodes_in_coord_relays(coord_relays, relay_peer, &nodes);
        } else {
    ...
    if let Some((peer_str, addrs)) = all_relays.first() {
        if let Ok(relay_peer) = peer_str.parse::<PeerId>() {
            ...
            if let Ok(mut g) = session.ghal_bol_relay_state.write() {
    ...
    if relay_circuit_listening(swarm) {
        let bootstrap_ok = ghal_bol_relay_peer(session)
