    ...
    let (peer, frame, done) = match cmd {
        ...
        OutboundCmd::SendText {
            ...
            if crate::text_transport::wan_text_via_delivery_server() {
                ...
                if !session.peer_on_local_lan(peer) {
                    return Err(
                        "WAN text uses delivery server (set GHAL_BOL_DELIVERY_URL; peer not on LAN)"
                            .into(),
                    );
                }
            }
        ...
        OutboundCmd::SendAck {
            ...
            if crate::text_transport::wan_text_via_delivery_server() {
                ...
                if !session.peer_on_local_lan(peer_id) {
                    return Err(
                        "WAN text acks use delivery server (peer not on LAN)".into(),
                    );
                }
            }
            let peer_id = session
