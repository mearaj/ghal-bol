...
6. **Wi‑Fi toggle on LAN** — **event-driven** ([TRANSPORT.md](TRANSPORT.md) § “Event-driven async”): connectivity notify → **`kick_lan` once**; mDNS `Discovered` → dial; connect/disconnect/fail → notify subscribers (`notify_stream_reopen`, `notify_coord_lookup`). **Do not** pause all coord for a grace timer or poll recovery on `dm_upkeep`. Defer coord relay only while **LAN dial in flight** or **waiting for first mDNS TCP after purge** (state flags). LAN dials remain **mDNS `Discovered` only** — never upkeep re-dial from cache.
