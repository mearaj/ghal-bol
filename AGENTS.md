...
- **Identity tab `SelectableText` / `SelectionArea` + stacked dialogs** — plain `Text` on `_identityBody`; dialog controllers in **`StatefulWidget` `State.dispose()`** only — never `ctrl.dispose()` right after `await showDialog` (`TextEditingController was used after being disposed` → `_dependents.isEmpty` cascade). Pattern: `invite_paste_dialog.dart`, `identity_key_management.dart` `_AppPasswordDialog`. DESIGN.md § “Identity tab — Show private key”.
...
- **Overlapping Android background permission prompts** — notification, battery optimization, unused-app pause, and OEM autostart must run only via **`AndroidBackgroundReadiness.runIfNeeded`** (hub unlock, **before** `P2pEventBridge.ensureStarted`). **`ghal_bol_listener_foreground_io.dart` must not** call `Permission.notification.request()` — that raced the hub flow and stacked system dialogs. One step at a time; skip steps already satisfied. DESIGN.md § “Fixed 2026-07-05 — Android background readiness”.
- **Assuming FGS + `WAKE_LOCK` alone keep `:p2p` alive with screen off** — stock Doze / “Pause app activity if unused” and OEM autostart layers can still throttle or freeze the UID despite `remoteMessaging` foreground service. Prompt for **`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`** + OEM settings when needed; do not move connect/ack policy to Dart `NetworkHelper`. Verified fix: DESIGN.md § “Fixed 2026-07-05”.
- **Device-model-specific background hacks** — use **`BackgroundReadiness.kt`** manufacturer detection + resolvable settings intents (Xiaomi, Oppo/Realme, Vivo, Huawei/Honor, OnePlus, Asus). Query autostart state when the OS exposes it (e.g. Vivo content provider); do not hardcode a single OEM model or adb-only workarounds in product code.
...
| Identity → Show private key → red screen (`_dependents.isEmpty` or `TextEditingController was used after being disposed`, debug) | **Regression:** password dialog disposed `TextEditingController` in caller after `showDialog` (primary — see `ghal_bol_ui/flutter_android.log`); and/or `SelectableText` on Identity `IndexedStack`. Fix: `_AppPasswordDialog` / `invite_paste_dialog.dart` pattern; plain `Text` on `_identityBody` — DESIGN.md § “Identity tab — Show private key”. |
