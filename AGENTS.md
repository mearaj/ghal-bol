...
| Linux desktop: read acks missing while chat visibly open | **Open bug** — hub→native read gate stale; resize may unblock post-frame layout sync. **Never** reintroduce forbidden `lastApplySucceeded` hub patch (broke P2P). DESIGN.md § “FORBIDDEN — reverted 2026-06-15” |
