...
| Linux: no messages after reboot/login until app manually opened | **XDG autostart:** `~/.config/autostart/com.ghalbol.daemon.desktop` starts `ghal_bol_daemon` on login. After 10 s grace, if still locked, daemon shows `notify-rust` notification ("Enter your password…"). Click → `gtk-launch com.ghalbol` → app opens → unlock. If no autostart: check file exists and `Exec=` path is correct. If no notification: rebuild native (`session_unlocked` export). DESIGN.md § "Linux desktop — daemon auto-start and unlock notification". |
