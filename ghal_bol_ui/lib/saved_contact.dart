...
class SavedContact {
  ...
  SavedContact copyWith({
    ...
    return SavedContact(
      ...
      lastMessagePreview: clearLastMessage
          ? null
          : (lastMessagePreview ?? this.lastMessagePreview),
      lastMessageAtMs: clearLastMessage
          ? null
  ...
  static SavedContact? fromJson(dynamic raw) {
    ...
    if (!isValidPublicKeyHex(pk)) {
      ...
      if (legacyPid.isNotEmpty) {
        pk =
            GhalBolFfi.publicKeyHexFromPeerId(
              legacyPid,
            )?.trim().toLowerCase() ??
            "";
      }
    ...
    return SavedContact(
      ...
      availabilityStatus: ghalSanitizePeerAlias(
        raw["availability_status"]?.toString(),
      ),
      lastMessagePreview: raw["last_message_preview"]?.toString(),
      lastMessageAtMs: raw["last_message_at_ms"] is int
          ? raw["last_message_at_ms"] as int
          : null,
      unreadCount: raw["unread_count"] is int
          ? (raw["unread_count"] as int).clamp(0, 9999)
          : 0,
      ...
      createdAtMs: raw["created_at_ms"] is int
          ...
          : null,
      updatedAtMs: raw["updated_at_ms"] is int
          ? raw["updated_at_ms"] as int
          : null,
      chatRoomExitAtMs: raw["chat_room_exit_at_ms"] is int
          ? raw["chat_room_exit_at_ms"] as int
          : null,
    );
  }
