...
#   COORD_URL=http://127.0.0.1:8765 ./ghal_bol_coord/deploy/smoke_coord.sh
#   LIVE_ONLY=1 COORD_URL=... COORD_INSECURE_TLS=1 ./ghal_bol_coord/deploy/smoke_coord.sh  # skip cargo test
set -euo pipefail
...
if [[ "${LIVE_ONLY:-0}" != "1" ]]; then
  echo "== ghal_bol_coord: in-process API tests =="
  cargo test -p ghal_bol_coord --test http_api

  echo "== ghal_bol_coord: production E2E (subprocess + TCP + SQLite) =="
