...
| **Loopback smoke** | `cargo run -p ghal_bol_coord` | `:4002` default |
...
```bash
GHAL_BOL_RELAY_PUBLIC_HOST=coord1.ghalbol.com cargo run -p ghal_bol_coord
```
...
Same data root namespace as the coord server only (`com.ghal_bol.coord` — not the Flutter app `com.ghalbol`).
...
|--------|---------------------------|--------------|
| Client `POST /v1/register` | **Public routable IPv4 TCP** only (peer’s own inbound DM listen) | Heartbeat TTL expiry (`GHAL_BOL_COORD_PRESENCE_TTL_SECS`) |
